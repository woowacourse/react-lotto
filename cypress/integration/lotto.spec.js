...
describe('lotto-game', () => {
  ...
  const typeWinningNumbersAndClickShowResultButton = (inputNumbers = [5, 10, 15, 20, 25, 30, 35]) => {
  ...
  it('구입 금액에 3000원을 입력 시, 로또 3개가 자동으로 구입되야 한다.', () => {
    ...
    cy.get('#purchase-result-section__label').should('have.text', '총 3개를 구매하였습니다.');
  ...
  it('구입 금액에 1000원 이하의 값을 입력 시, 경고 메시지가 출력되야 한다.', () => {
    ...
    cy.get('@windowAlert').should('be.calledWith', ERR_MESSAGE.LOTTO.INVALID_PRICE);
  });
  ...
  it('당첨 번호에 중복된 번호을 입력시, 경고 메시지가 출력되야 한다.', () => {
    ...
    cy.get('@windowAlert').should('be.calledWith', ERR_MESSAGE.WINNING_NUMBER.DUPLICATE);
  });

  it('"1 ~ 45"가 아닌 번호를 입력시, 경고 메시지가 출력되야 한다.', () => {
    playLottoGame([5, 10, 15, 25, 30, 35, 50]);

    cy.get('@windowAlert').should('be.calledWith', ERR_MESSAGE.WINNING_NUMBER.OUT_OF_RANGE);
  });
